...
| **2026-05-19** | [`actions-cool` hijack](2026-05-actions-cool-hijack/) | Imposter commits via stolen maintainer creds, secrets scraped from `/proc/<pid>/mem` | `actions-cool/*` Actions | Prevention (source policy) + Content (secretscan, 6 findings) + Behavior (`/proc/*/mem` + `/proc/self/environ` + `/tmp/runner_collected_*`) | ✅ **live detection** — all 3 layers green |
